...
shiftDig :: Integer -> Integer -> Integer
shiftDig prefix' suffix' = 10 * prefix' + suffix'

eval :: [Integer] -> Integer
eval as = foldr shiftDig 1 as
...
toDigitsRev x | quot' == 0 = rem' : []
              | otherwise = rem' : toDigitsRev quot'
...
doubleSecond as = map (\(x, i) -> if i `mod` 2==1 then 2*x else x ) (zipWithIndex as)
